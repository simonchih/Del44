The name of game is "Deletion 44" as it can delete 44 neighbor stones with same color at most.

==Rules==
1. Select a source stone by mouse button clicked.
2. Then, select a target stone that neighbors a source stone (up, down, left, right) by mouse button clicked. 
3. If target stones have more than 3 stones as same color in a row or in a column, copy the color of source stone to target stones. Otherwise, go to step 1.
4. Delete stones if there are 5 stones or great have same color in a row or in a column.

==Chinese Readme==

這個遊戲我取名叫Deletion 44，因為一次最多消去44個相鄰的同色石頭。

玩法如下：
1. 用滑鼠選擇一個石頭，當作來源石
2. 接下來，用滑鼠選擇一個來源石旁邊(上、下、左、右)的石頭，當作目標石
3. 如果目標石的行或列，存在同顏色的連續三個以上，就把來源石複製到目標石，否則回到step 1.
4. 當行或列，存在5個或以上，同顏色的石頭，就會消除石頭
